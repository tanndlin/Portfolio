...
function Project(props) {
    return (
        <section className=''>
            <div>
                <h1 className={`text-2xl mb-16 font-mono ${props.side === 'right' && 'text-right'}`}>
                    {props.title}
                </h1>
                <div className='relative'>
                    ...
                    <footer className={`${props.side === 'right' ? 'float-right' : ''} flex flex-col mt-4`}>
                        ...
                        <a className={`${props.side === 'right' ? 'justify-end' : ''} flex mt-4 gap-4 w-min h-min hover:fill-white`} href={props.link}>
